...
func TestReElection2A(t *testing.T) {
	...
	cfg.begin("---------------leader disconnects---------------")
	cfg.disconnect(leader1)
	cfg.checkOneLeader()

	cfg.begin("---------------old leader rejoin---------------")
	...
	fmt.Printf("---------------disconnect leader %d %d--------------\n", leader2, (leader2 + 1) % servers)
	...
	fmt.Printf("--------------- connect1  %d--------------\n", (leader2 + 1) % servers)
	
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	fmt.Printf("--------------- connect2  %d--------------\n", leader2)
	...
	cfg.end()
	cfg.begin("---------------Test (2A): election after network failure end---------------")
}
...
func TestManyElections2A(t *testing.T) {
	...
	cfg.end()
	cfg.begin("---------------Test (2A): multiple elections end---------------")
...
func TestFollowerFailure2B(t *testing.T) {
	...
	fmt.Printf("-------------- disconnect follower %d  ------------\n", (leader1 + 1) % servers)
	...
	fmt.Printf("-------------- disconnect the remaining follower  ------------\n")
	...
	fmt.Printf("-------------- check that command 104 did not commit.  ------------\n")
...
func TestFailAgree2B(t *testing.T) {
	...
	fmt.Printf("-------------- follower %d reconnect ------------\n", (leader + 1) % servers)
...
func TestFailNoAgree2B(t *testing.T) {
	...
	fmt.Printf("-------------- reconnect follower %d %d %d------------\n", (leader + 1) % servers, (leader + 2) % servers, (leader + 3) % servers)
	// repair
	cfg.connect((leader + 1) % servers)
...
func TestConcurrentStarts2B(t *testing.T) {
	servers := 3
	fmt.Printf("-------------- start create %d servers ------------\n", servers)
...
loop:
	for try := 0; try < 5; try++ {
		...
		fmt.Printf("-------------- try= %d, check one leader ------------\n", try)
		leader := cfg.checkOneLeader()
		fmt.Printf("-------------- try= %d, check one leader over ------------\n", try)
		...
		fmt.Printf("-------------- get the expect environment ------------\n")
...
func TestRejoin2B(t *testing.T) {
	...
	fmt.Printf("-------------- new leader commit 103 ------------\n")
	...
	fmt.Printf("-------------- disconnect leader %d ------------\n", leader2)
	fmt.Printf("-------------- old leader %d rejoin ------------\n", leader1)
...
func TestBackup2B(t *testing.T) {
	...
	// }
	fmt.Printf("-------------- submit 50 command over ------------\n")
	...
	fmt.Printf("-------------- curr partion disconnect ------------\n")
	fmt.Printf("-------------- allow other partition to recover %d %d %d------------\n",(leader1 + 2) % servers, (leader1 + 3) % servers, (leader1 + 4) % servers)
	// lots of successful commands to new group.
	fmt.Printf("-------------- commit 50 command ------------\n")
	for i := 0; i < 50; i++ {
		cfg.one(50+i, 3, true)
	}
	// for i := 0; i < 50; i++ {
	...
	// }
	fmt.Printf("-------------- commit 50 command over------------\n")
	...
	fmt.Printf("-------------- commit 50 command 2------------\n")
	// lots of successful commands to new group.
	for i := 0; i < 50; i++ {
...
func TestPersist12C(t *testing.T) {
	...
	for i := 0; i < servers; i++ {
		fmt.Printf("-------------- crash and restart %d ------------\n", i)
		cfg.start1(i, cfg.applier)
	}
	for i := 0; i < servers; i++ {
		fmt.Printf("-------------- disconnect and reconnect %d ------------\n", i)
...
func TestFigure8Unreliable2C(t *testing.T) {
	...
	for iters := 0; iters < 1000; iters++ {
		...
		for i:=0; i<5; i++{
			if cfg.connected[i]{
				fmt.Printf("-------------- server in connect: S%d ------------\n", i)
				
			}
		}
		fmt.Printf("-------------- iters: %d over ------------\n", iters)
