...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	// 		// 没投过票或者投票请求来自高term --> 投票 并转为 Follower
	// 		rf.state = Follower
	// 		rf.votedFor = args.Id
	// 		rf.term = args.Term
	// 		rf.stateReset = true
	...
	// 		reply.Term = rf.term
	// 		if args.Index >= rf.lastApplied {
	// 			reply.GetVote = true
	// 			DebugLog(dVote, "S%d [%d] Follower vote --> %d", rf.me, rf.term, args.Id)
	// 		} else {
	// 			reply.GetVote = false
	// 			DebugLog(dVote, "S%d [%d] Follower not vote --> %d", rf.me, rf.term, args.Id)
	...
	// 	// Candidate 收到投票请求
	// 	if args.Term > rf.term {
	// 		// 来自高term --> 投票, 转为Follower
	// 		rf.state = Follower
	// 		rf.votedFor = args.Id
	// 		rf.term = args.Term
	// 		reply.Term = rf.term
	// 		reply.GetVote = true
	// 		DebugLog(dVote, "S%d [%d] Candidate (term:%d) vote --> %d (term:%d)", rf.me, rf.term, rf.term, args.Id, args.Term)
	// 	} else if args.Term == rf.term && args.Index > rf.lastApplied {
	// 		// 来自同term高index --> 投票, 转为Follower
	// 		rf.state = Follower
	// 		rf.votedFor = args.Id
	// 		rf.term = args.Term
	// 		reply.Term = rf.term
	// 		reply.GetVote = true
	// 		DebugLog(dVote, "S%d [%d] Candidate (index:%d) vote --> S%d [%d] (index:%d)", rf.me, rf.term, rf.lastApplied, args.Id, args.Term, args.Index)
	// 	// 	} else if args.Term == rf.term && args.Index == rf.lastApplied && args.CommitIndex >{
	...
	// 		// DebugLog(dVote, "S%d [%d] Candidate not vote", rf.me, rf.term)
	// 	} 
	// } else {
	// 	// leader 
	// 	// if args.Term > rf.term && args.Index >= rf.lastApplied{
	// 	if args.Term > rf.term {
	// 		rf.state = Follower
	// 		rf.votedFor = args.Id
	// 		rf.term = args.Term
...
func (rf *Raft) followerHandleAE(args *AppendEntries, reply *AppendEntriesReply) {
	...
	if rf.lastApplied > args.PreLogIndex{
		...
		if rf.log[args.PreLogIndex].Term == args.PreLogTerm {
			...
			DebugLog(dLog, "S%d [%d] Follower, pre not match, log not match, delete pre and log %v", rf.me, rf.term, rf.getLogInfo(rf.log))
			reply.Success = false
			reply.IsExist = false
			reply.MatchIndex = rf.lastApplied
		}
		...
		if rf.lastApplied == 0 || rf.log[args.PreLogIndex].Term == args.PreLogTerm {
			...
			// not match
			DebugLog(dLog, "S%d [%d] Follower, return false, pre not match log not match, rf.lastApplied= %d, args.PreLogIndex= %d, LOG : %v", rf.me, rf.term, rf.lastApplied, args.PreLogIndex, rf.getLogInfo(rf.log))
			// delete entry
		...
		// rf.lastApplied < args.PreLogIndex
		// preIndex not exist
		DebugLog(dLog, "S%d [%d] Follower, return false, rf.lastApplied = %d < PreLogIndex = %d LOG: %v", rf.me, rf.term, rf.lastApplied, args.PreLogIndex, rf.getLogInfo(rf.log))
...
func (rf *Raft) SendAEToPeer(id int) {
	...
	for {
		...
		case <-ticker.C:
			...
			DebugLog(dTimer, "Leader, retry send ")
		}
		ticker.Stop()
...
func (rf *Raft) SendOneAE(id int) {
	...
	if(rf.nextIndex[id]==rf.lastApplied+1){
		// first heartBeat or has matched over
		DebugLog(dTimer, "S%d [%d] Leader, send empty heartBeat to %d, args.PreLogIndex = %d", rf.me, rf.term, id, args.PreLogIndex)
	...
	if ok {
		...
		if reply.Success {
			if reply.IsExist {
				DebugLog(dLog, "S%d [%d] Leader this log is repeat", rf.me, rf.term)
				rf.mu.Unlock()
				return
			}
			rf.MatchIndex[id] = reply.MatchIndex
